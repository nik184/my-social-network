...
async function loadAvatarImages() {
    try {
        const response = await fetch('/api/media/image/galleries');
        const data = await response.json();

        // Extract images from the avatar gallery
        const galleries = data.galleries || [];
        const avatarGallery = galleries.find(g => g.name === 'avatar');
        const images = avatarGallery ? avatarGallery.files : [];
        
        avatarImages = images;
        return {images: images, count: images.length, primary: images[0]};
    } catch (error) {
...
function updateHeaderAvatar(avatarDisplay) {
    if (avatarImages.length > 0) {
        avatarDisplay.innerHTML = `<img src="/api/media/image/galleries/avatar/${avatarImages[0]}" alt="Avatar" class="avatar">`;
    } else {
...
function showGalleryImage() {
    ...
    const imageUrl = galleryUrlProvider ? galleryUrlProvider(galleryImages[currentGalleryIndex], currentGalleryIndex) :
        galleryType === 'avatar' ? `/api/media/image/galleries/avatar/${galleryImages[currentGalleryIndex]}` :
